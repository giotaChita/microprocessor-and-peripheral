...
void toggle_led_isr(){				//toggle led every seconds
	...
	do{
		...
		printf("led is %s\n", gpio_get(P_LED_R) > 0 ?"ON":"OFF");
		sprintf(led_str, "led is %s\r\n", gpio_get(P_LED_R) > 0 ?"ON":"OFF");
		uart_print(led_str);
		if(counter == period - 1)
			break;
...
void uart_rx_isr (uint8_t rx ){
	if(first_time_aem){
		...
		if(rx == 13){ //if ENTER was pressed
			...
			if (last_index == 4) printf("and aem is %d%d%d%d%d\n", uart_input[0]-'0', uart_input[1]-'0', uart_input[2]-'0', uart_input[3]-'0', uart_input[4]-'0');
			else if (last_index == 3)	printf("and aem is %d%d%d%d\n", uart_input[0]-'0', uart_input[1]-'0', uart_input[2]-'0', uart_input[3]-'0');
...
int main(void){
	...
	//uart_rx();
	while(first_time_aem);									//wait untill aem is given! Then the flag will drop.
	...
	do{
		if (counter >= period){
			char period_str[100];
			sprintf(period_str, "twra pernoyn %d sec\r\n", period);
			printf("%s", period_str);			//printf("twra pernoyn %d sec\n", period);
			uart_print(period_str);				//uart_print("perasan kapoia sec\n");
	...
	do{
		if (counter >= period){							// take temperature
			char period_str2[100];
			sprintf(period_str2, "twra pernoyn %d sec\n", period);
			printf("%s", period_str2);			//printf("twra pernoyn %d sec\n", period);
			uart_print(period_str2);				//uart_print("perasan kapoia sec\n");
